...
def test_mobile_obj(mobile_os_name, mobile_os_version, device_name, app_package, app_activity, remote_flag, device_flag, testrail_flag, tesults_flag, test_run_id, app_name, app_path):
    ...
    test_mobile_obj.register_driver(mobile_os_name, mobile_os_version, device_name, app_package, app_activity,
                                    remote_flag, device_flag, app_name, app_path, ud_id, org_id, signing_id, no_reset_flag)
...
def pytest_terminal_summary(terminalreporter, exitstatus):
    ...
    if terminalreporter.config.getoption("--email_pytest_report").lower() == 'y':
        ...
        email_obj.send_test_report_email(
            html_body_flag=True, attachment_flag=True, report_file_path='default')
    ...
    if 'browser' in metafunc.fixturenames:
        if metafunc.config.getoption("-M").lower() == 'y':
            if metafunc.config.getoption("-B") == ["all"]:
                ...
                config_list = [(metafunc.config.getoption("-B")[0], metafunc.config.getoption(
                    "--ver")[0], metafunc.config.getoption("-P")[0], metafunc.config.getoption("-O")[0])]
                metafunc.parametrize("browser,browser_version,os_name,os_version",
                                     config_list)
        if metafunc.config.getoption("-M").lower() != 'y':
            if metafunc.config.getoption("-B") == ["all"]:
                ...
                metafunc.parametrize(
                    "browser", browser_os_name_conf.default_browser)
            else:
